...
class LoginController extends BaseController {
    ...
    public function check_login(){
        if(!IS_POST){
            ...
            $temp = $_POST;
            $isset = M('sysuser')->where(array('sysu_account'=>$temp['account'],'sysu_pwd'=>sha1(md5($temp['password']))))->select();
            if(sizeof($isset)==1){
                $data = array('success'=>true,'msg'=>'验证通过');
                session('account',$temp['account']);
                session('sysu_id',$isset[0]['sysu_id']);
                session('role_id',$isset[0]['role_id']);
            }else{
