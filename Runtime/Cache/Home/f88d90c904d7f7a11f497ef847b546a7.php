...
<body>
    <div class="school_content">
        ...
        <table class="layui-table" lay-even>
            ...
            <tbody>
                <?php if(is_array($Data['user'])): foreach($Data['user'] as $key=>$v): ?><tr>
                        <td><?php echo ($v['user_name']); ?></td>
                        <td><?php echo ($v['user_account']); ?></td>
                        <td class="limit_length"><?php echo ($v['comment']); ?></td>
                        <td>
                            <a class="layui-btn layui-btn-normal layui-btn-small resetPwd" name="<?php echo ($v['user_id']); ?>">
                                <i class="layui-icon">&#xe640;</i>重置密码</a>
                            <!-- <a class="layui-btn layui-btn-warm layui-btn-small delete" name="<?php echo ($v['sysu_id']); ?>">
                                <i class="layui-icon">&#xe640;</i>删除</a> -->
                            <?php if(($v['state']) != "1"): ?><a class="layui-btn layui-btn-danger layui-btn-small motivate" name="<?php echo ($v['user_id']); ?>">
                                <i class="layui-icon">&#xe640;</i>激活</a><?php else: ?><a class="layui-btn layui-btn-success layui-btn-small lock" name="<?php echo ($v['user_id']); ?>">
    ...
    var sysuInfo = "<?php echo U('User/User_index');?>";
    var switchState = "<?php echo U('User/User_SwitchState');?>";
    var resetPwd = "<?php echo U('User/User_ResetPwd');?>";
    var check_modify = "<?php echo U('News/check_modify');?>";
    var modify_news = "<?php echo U('News/News_modifyNews');?>";
    var SysUser_delete = "<?php echo U('User/User_delete');?>";
